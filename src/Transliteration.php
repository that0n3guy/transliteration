...
class Transliteration {
     ...
     */
    public function clean_filename($filename, $source_langcode = NULL) {
        if (is_array($filename)) {
            foreach ($filename as $key => $value) {
                $filename[$key] = $this->clean_filename($value, $source_langcode);
            }
            return $filename;
        }

        $filename = $this->transliteration_process($filename, '', $source_langcode);
        // Replace whitespace.
...
//        }
     ...
     *   Transliterated text.
     */
    public function transliteration_process($string, $unknown = '?', $source_langcode = NULL) {
        ...
        foreach ($matches[0] as $str) {
            ...
            for ($i = -1; --$len; ) {
                ...
                if ($remaining = $tail_bytes[$c]) {
                    ...
                    $result .= $this->transliteration_replace($ord, $unknown, $source_langcode);
                    $head = '';
                }
                elseif ($c < "\x80") {
                    // ASCII byte.
     ...
     */
    public function transliteration_replace($ord, $unknown = '?', $langcode = 'eng') {
        static $map = array();

        $bank = $ord >> 8;

        if (!isset($map[$bank][$langcode])) {
            $file = realpath(dirname(__FILE__)) . '/data/' . sprintf('x%02x', $bank) . '.php';
